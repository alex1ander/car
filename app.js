...
const isLooping      = true;    // Зациклить анимацию
const isStart        = true;    // Начальные позиции блоков при загрузке
const showCoords     = true;    // Отображать координаты блоков

// ===== Конфигурация сцены и блоков =====
const CONFIG = {
  block: {
    width: 1.6,
    depth: 1.6,
...
outerGroup.rotation.y = deg(CONFIG.TILT_Y);
...
function displayCoords() {
  ...
  panels.forEach(p=>{
    console.log(`${p.id} → x:${p.mesh.position.x.toFixed(2)}, y:${p.mesh.position.y.toFixed(2)}, z:${p.mesh.position.z.toFixed(2)}`);
  });
}
...
function animate(now){
  ...
  if(phase===PHASE.ASSEMBLE){
    ...
    panels.forEach(p=>{ lerpV(p.mesh.position,p.startPos,p.endPos,t); lerpR(p.mesh.rotation,p.startRot,p.endRot,t); });
    if(phaseTime>=CONFIG.assembleDuration){ phase=PHASE.HOLD; phaseTime=0; displayCoords(); }
  } else if(phase===PHASE.HOLD){
    if(!isLooping) return;
    if(phaseTime>=CONFIG.holdDuration){ phase=PHASE.DISASSEMBLE; phaseTime=0; }
  } else if(phase===PHASE.DISASSEMBLE){
    const t=easeIn(Math.min(phaseTime/CONFIG.disassembleDuration,1));
    panels.forEach(p=>{ lerpV(p.mesh.position,p.endPos,p.startPos,t); lerpR(p.mesh.rotation,p.endRot,p.startRot,t); });
    if(phaseTime>=CONFIG.disassembleDuration){ 
      phase=PHASE.PAUSE; phaseTime=0; 
      panels.forEach(p=>{ p.mesh.position.copy(p.startPos); p.mesh.rotation.copy(p.startRot); });
      displayCoords();
    }
  } else if(phase===PHASE.PAUSE){
    if(phaseTime>=0.8){ phase=PHASE.ASSEMBLE; phaseTime=0; }
  }
